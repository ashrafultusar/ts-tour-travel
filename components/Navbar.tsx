...
export default async function Navbar() {
    ...
    return (
        <nav className="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-slate-100 shadow-sm">
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="flex h-20 justify-between items-center">
                    <div className="flex items-center gap-8">
                        {/* Mobile Menu Toggle Button (Left aligned for mobile/tablet) */}
                        <label htmlFor="mobile-menu-toggle" className="p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer lg:hidden">
                        ...
                        <div className="flex items-center">
                            {session?.user ? (
                                ...
                                <div className="flex items-center gap-2 sm:gap-4">
                                    <Link href="/login" className="hidden xs:block text-sm font-bold text-[#1e293b] hover:text-[#1665a1]">
                                        Log in
                                    </Link>
                                    <Link href="/register" className="rounded-xl bg-gradient-to-r from-[#1665a1] to-[#0891B2] px-4 sm:px-6 py-2.5 text-xs sm:text-sm font-bold text-white shadow-md hover:opacity-90 transition-all">
                                        Consultation
                                    </Link>
                                </div>
                            )}
            ...
            {/* Mobile Side Nav Drawer (Left Aligned) */}
            <div className="fixed inset-0 z-[100] invisible opacity-0 peer-checked/menu:visible peer-checked/menu:opacity-100 transition-all duration-300">
                ...
                {/* Sidebar Panel - Now Left Aligned with translate-x-[-100%] */}
                <div className="absolute top-0 left-0 h-full w-[280px] bg-white shadow-2xl transform translate-x-[-100%] peer-checked/menu:translate-x-0 transition-transform duration-300 ease-in-out border-r border-slate-100">
                    <div className="flex flex-col h-full">
                        ...
                        <div className="flex-1 overflow-y-auto py-4">
                            {session?.user && (
                                <div className="px-6 py-4 mb-2 bg-slate-50/80 border-y border-slate-100/50">
                                    ...
                                    <Link href="/profile" className="flex items-center gap-2 text-xs font-bold text-[#1665a1]">
                            ...
                            {navLinks.map((link) => (
                                <Link key={link.name} href={link.href} className="flex items-center justify-between px-6 py-4 text-slate-700 hover:bg-slate-50 hover:text-[#1665a1] group transition-all">
                        ...
                        <div className="p-6 border-t border-slate-100 space-y-4 bg-slate-50/50">
                            {session?.user ? (
                                <LogoutButton className="w-full flex items-center justify-center gap-2 bg-red-50 text-red-600 py-3 rounded-xl font-bold transition-colors" />
                            ) : (
                                <Link href="/register" className="block w-full text-center rounded-xl bg-gradient-to-r from-[#1665a1] to-[#0891B2] py-3.5 text-white font-bold shadow-md">
                                    Book Consultation
                                </Link>
                            )}
                        </div>
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                /* Sidebar Panel inside the container */
                #mobile-menu-toggle:checked ~ div > div:last-child { transform: translateX(0); }
                
                #profile-dropdown-toggle:checked ~ div .peer-checked\\/profile\\:visible { 
                    visibility: visible; 
                    opacity: 1; 
                }
            `}} />
