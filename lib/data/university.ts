...
// Data Cache
const getUniversitiesConfig = async (page: number, limit: number, search?: string, location?: string, level?: string | string[], offerType?: string | string[]) => {
    return await unstable_cache(
        async () => {
            ...
            const plainUniversities = universities.map((uni) => serializeDocument(uni));
...
export const getUniversities = cache(async (page = 1, limit = 10, search?: string, location?: string, level?: string | string[], offerType?: string | string[]) => {
    return await getUniversitiesConfig(page, limit, search, location, level, offerType);
});
